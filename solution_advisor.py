...
# Title and Description
st.title("Solution Advisor")
st.write("Input your Roadmap or upload a file to start talking with the AI chatbot!")

# Layout: Two Columns
col1, col2 = st.columns(2)

with col1:
    st.header("Behavior Prompt (Optional)")
    behavior_prompt = st.text_area(
        "Enter your prompt here (Optional):",
        placeholder="E.g., Explain AI adoption challenges. Leave blank to use the default prompt."
    )

with col2:
    st.header("Roadmap")
    roadmap_text = st.text_area(
        "Enter your roadmap here:",
        placeholder="E.g., Timeline, milestones, etc."
    )
    uploaded_file = st.file_uploader("Upload Roadmap File (Optional)", type=["txt", "pdf", "docx"])
...
if st.button("Submit"):
    ...
    # Check if Roadmap is filled (it's required)
    if not roadmap_content:
        st.warning("Please enter or upload a Roadmap.")
    else:
        # Payload
        payload = {
            "behavior_prompt": behavior_prompt or None,  # Send None if empty
            "roadmap": roadmap_content
        }
